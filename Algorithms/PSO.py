...
class ParticleSwarmOptimizationFleet:
    ...
    def __init__(self, env, use_idleness = True):
        ...
        self.c_exploration = (0.5, 0, 3.2697, 0) 
        # self.c_exploration = (0.5, 0, 4, 0) 
        self.c_explotation = (3.6845, 1.5614, 0, 3.6703) 
        # c1 = best_local_locations, c2 = model_max_trash, c3 = closer_unexplored, c4 = closer_trash
    ...
    def update_vectors(self):
        ...
        for agent_id, agent_position in self.active_agents_positions.items():
            ...
            # Update best historic local measure and its locations. The best location is the one with the most trash at sight. #
            ...
            if np.any(self.model_trash_map > 0):
            ...
            self.velocities[agent_id] = (self.velocities[agent_id] / (np.linalg.norm(self.velocities[agent_id]) + 0.01) ) * self.movement_length_of_each_agent[agent_id] # 0.01 to avoid division by zero
